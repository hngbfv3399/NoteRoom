...
function ProfileHeader({ currentTime, emotionTemp }) {
  ...
  const { text: emotionText, color: emotionColor } = getEmotionTextAndColor(emotionTemp);

  return (
    <div className="absolute top-6 left-1/2 transform -translate-x-1/2 flex flex-col items-center space-y-2 z-20">
      <p className="text-5xl font-bold text-white drop-shadow-lg">{currentTime}</p>
      <p className={`text-3xl font-semibold drop-shadow-lg ${emotionColor}`}>{emotionTemp}°C</p>
      <p className={`text-2xl font-semibold drop-shadow-lg ${emotionColor}`}>{emotionText}</p>
    </div>
  );
}
